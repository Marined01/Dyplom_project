...
У Docker використовується **сервер розробки Django** (`runserver`) — достатньо для демонстрації та розробки. Для промислового розгортання потрібен WSGI-сервер (наприклад, Gunicorn) і reverse-proxy.
